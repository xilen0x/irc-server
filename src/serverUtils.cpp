...
void acceptNewClient(int _fdServer, std::vector<struct pollfd>& _fdsClients)
{
    ...
    int fdClient = accept(_fdServer, reinterpret_cast<struct sockaddr*>(&clientAddress), &clientAddressSize);
    if (fdClient == -1) {
    ...
    // Add the client to the list of monitored file descriptors
